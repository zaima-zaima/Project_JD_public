  ...
  <div class="publish-container">
    ...
    <div class="form">
      ...
      >
        ...
        <div class="uploads">
          <el-form-item label="封面上传" prop="thumbs">
            <Uploads
              :limit="3"
              sigle="goods"
              @upload="uploadHandle"
          ...
          <el-form-item label="商品描述图片" prop="desc">
            <Uploads
              :limit="8"
              sigle="production"
              @upload="uploadProduction"
              :accepts="state.accepts"
            />
          </el-form-item>
        </div>
      ...
      >
        ...
        <el-form-item label="成分" prop="ingradient">
          <el-input v-model="state.form.ingradient" />
        </el-form-item>
        <el-form-item label="适用人群" prop="approperate">
          <el-select v-model="state.form.approperate" placeholder="请选择">
        ...
        <el-form-item label="国产/进口" prop="region">
          <el-select v-model="state.form.region" placeholder="请选择">
            <el-option label="国产" value="internal" />
            <el-option label="进口" value="external" />
          </el-select>
        </el-form-item>
        <el-form-item label="规格" prop="specification">
          <el-select v-model="state.form.specification" placeholder="请选择">
...
import rules from "../../../utils/validators/rulers";
...
async function submit() {
  form2.value.validate(async (valid: any) => {
    if (valid) {
      state.loading = true;
      state.form.owner = store.state.user.id;
      const resp = (await goodsCreator(state.form)) as unknown as Goods | any;
      if (!resp.code) {
        state.loading = false;
        state.success = true;
        state.current = 3;
